...
The reason it takes so long to load `/customers` after uploading our file is that we are processing the file on the same thread as the request/response. File processing becomes a *blocking* action on the thread, and nothing else can happen until that's done.
...
To see it in action, you'll first need to start Redis(in one tab):

`redis-server`

Then run your Rails server (don't forget to restart) in a second tab, and open a third tab to run Sidekiq with this command:
