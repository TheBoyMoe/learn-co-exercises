...
If inheritance describes an is-a relationship, then you can think of composition as describing a has-a relationship. Composition is using objects as parts of other objects, e.g. if a Car has a property engine property, and that property holds an Engine object with its own properties and methods, that's composition.  An Engine would never inherit from Car, and a Car would never inherit from Engine, but we can use an Engine (and other objects) to compose a Car.
